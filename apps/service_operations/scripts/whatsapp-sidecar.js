...
async function ensureClient(visibleLogin = false, force = false) {
  ...
  starting = (async () => {
    ...
    nextClient.on('authenticated', () => {
      if (currentGeneration !== generation) return;
      // V6.6.10 WhatsApp-only fast-auth behavior: the phone/QR link is already
      // accepted here. Do not keep Settings waiting for whatsapp-web.js to finish
      // its internal sender injection. The first send waits safely for that later
      // transport-ready event in the background.
      transportReady = false;
      loginBrowserOpen = false;
      loadingMessage = 'WhatsApp linked. Sender is warming in the background';
      mark('READY');
      void writeLinkedMarker().catch(() => {});
...
async function waitForTransportReady(maxMs = 30000) {
  const deadline = Date.now() + Math.max(1000, maxMs);
  while (Date.now() < deadline) {
    if (client && transportReady) return true;
    if (['LOGIN_REQUIRED','DISCONNECTED','ERROR'].includes(state)) return false;
    await new Promise(resolve => setTimeout(resolve, 120));
  }
  return Boolean(client && transportReady);
}
...
async function sendCustomerMessage(body) {
  ...
  if (!transportReady) {
    const becameReady = await waitForTransportReady(30000);
    if (!becameReady) {
      return { status: 'FAILED', detail: 'WhatsApp is linked, but the sender is still preparing. Please retry in a few seconds.' };
    }
  }
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') {
      return sendJson(res, 200, {
        ...
        ok: !runtimeError, state, ready: transportReady, loginRequired: ['NOT_STARTED','LOGIN_REQUIRED','DISCONNECTED','ERROR'].includes(state), runtimeError
      });
    }
