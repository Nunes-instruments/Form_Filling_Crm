...
function Time-Url([string]$u){$sw=[Diagnostics.Stopwatch]::StartNew();try{$r=Invoke-WebRequest -UseBasicParsing -TimeoutSec 8 -Uri $u;$sw.Stop();Write-Host ("{0} -> HTTP {1} in {2} ms" -f $u,[int]$r.StatusCode,$sw.ElapsedMilliseconds) -ForegroundColor Green}catch{$sw.Stop();Write-Host ("{0} -> FAILED after {1} ms : {2}" -f $u,$sw.ElapsedMilliseconds,$_.Exception.Message) -ForegroundColor Red}}
