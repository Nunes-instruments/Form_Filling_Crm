...
export default function Shell({children}:{children:ReactNode}){
 ...
 return <div className={`shell ${path==="/"?"dashboard-shell":""}`}>
  <aside className={`sidebar ${mobile?"mobile-open":""}`}><div className="brand"><div className="brandmark">N</div><div><b>NUNES</b><span>Operations Workspace</span></div><button className="mobile-close" onClick={()=>setMobile(false)}><X/></button></div><nav className="nav">{nav.map(n=>{const I=n.href==="/"&&path==="/"?LayoutDashboard:n.icon;return <Link key={n.href} href={n.href} prefetch={false} onClick={()=>setMobile(false)} className={active(n.href)?"active":""}><I/><span>{n.label}</span></Link>})}</nav><div className="side-foot">{path==="/"?<><b>Management workspace</b><span>Live Purchasing and Servicing operations in one clear owner view.</span><em>NUNES · V6.5.0</em></>:<><b>Modular company platform</b><span>Purchasing and Servicing are the first two operational modules. New applications can be added through the module registry.</span><em>V6.5.0</em></>}</div></aside>
