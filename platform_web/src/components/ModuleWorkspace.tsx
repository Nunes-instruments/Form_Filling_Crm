...
export default function ModuleWorkspace({kind}:{kind:"purchasing"|"servicing"}){
 ...
 return <div className="module-shell"><div className="module-head"><Link href="/forms" className="btn"><ArrowLeft/>Forms</Link><div className="grow"><b>{title}</b><span>{ready?"Ready":msg}</span></div><button className="btn" onClick={reload}><RefreshCw/>Reload</button>{url&&<a className="btn" href={url} target="_blank" rel="noreferrer"><ExternalLink/>Open full screen</a>}</div><div className="module-frame-wrap">{ready&&url?<iframe key={frameKey} className="module-frame" src={url} title={title}/>:<div className="module-start-overlay static"><div className="spinner"/><b>{msg||"Starting…"}</b><p>The form is being kept warm in the background and will open automatically.</p></div>}</div></div>
}
